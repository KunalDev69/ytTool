...
#     for f in info.get('formats', []):
#         if f.get('format_id') in seen:
#             continue
#         seen.add(f.get('format_id'))

#         # Ensure we select the best format for video and audio
#         if f.get('vcodec') != 'none' and f.get('acodec') != 'none' and f.get('height') == 1080:
#             formats.append(f)

#     if not formats:
#         return render_template('error.html', message="❌ No suitable video found with video and audio in HD.")

#     # Assuming the first format is the best option
...
#         # Ensure video has both audio and video, and check if it's HD (720p and above)
#         if ext not in ['mp4', 'webm', 'm4a'] or not format_id:
#             continue
#         if vcodec == 'none' or acodec == 'none':  # Skip formats with no audio or video
#             continue
#         if height and height < 720:  # Skip formats with resolution lower than 720p
#             continue
...
#     if not formats:
#         return render_template('error.html', message="⚠️ No suitable video found with video and audio in HD.")
...
def fetch():
    ...
    for f in info.get('formats', []):
        print(f"Format ID: {f.get('format_id')} | Extension: {f.get('ext')} | Height: {f.get('height')} | VCodec: {f.get('vcodec')} | ACodec: {f.get('acodec')}")
